...
process_file() {
    ...
    timeout $timeout_duration root -l -b -q "$ROOT_MACRO(\"$file\", \"$out\", \"$apply_sce\", \"$apply_yz\", \"$apply_elife\", \"$apply_recom\", \"$is_data\")" || {
...
while IFS= read -r ROOT_FILE; do
    ...
    process_file "$ROOT_FILE" "$OUTPUT_FILE" "APPLY_SCE" "APPLY_YZ" "APPLY_ELIFE" "APPLY_RECOM" "IS_DATA"&  # Run in the background
