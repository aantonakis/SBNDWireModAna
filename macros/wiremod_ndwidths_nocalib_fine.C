...
const TString kLabels[kNdims] = { "x", "y", "z", "txz", "tyz", "dqdx", "rr", "width" };
const TString kTitles[kNdims] = { "x (cm)", "y (cm)", "z (cm)", 
    "ThetaXZ (deg)", "ThetaYZ (deg)", "dQ/dx", "Residual Range (cm)", "Width" };
const Int_t kNbins[kNdims] = { 600, 600, 700, 360, 360, 50, 50, 200 };
const Double_t kXmin[kNdims] = { -300, -300, -100, -180, -180, 0, 0, 0 };
const Double_t kXmax[kNdims] = { 300, 300, 600, 180, 180, 5000, 600, 20 };



void wiremod_ndwidths_nocalib_fine(const char* input_file, const char* output_file) {
	      ...
	      unsigned short east_tpc = 0;
		      ...
		      trk_thyz = TMath::ATan(*trk_diry / *trk_dirz) * 180. / TMath::Pi();
                    ...
                    */
                    ...
                    Double_t val[kNdims]  = {
                        sp.X(), sp.Y(), sp.Z(), trk_thxz, trk_thyz, dqdx_hit, rr_hit, width[ip][i] 
                    };
