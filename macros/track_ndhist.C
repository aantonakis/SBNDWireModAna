...
const TString kLabels[kNdims] = { "sx", "sy", "sz", "ex", "ey", "ez", "phi", "theta"};
const TString kTitles[kNdims] = { "start x (cm)", "start y (cm)", "start z (cm)", 
                                  "end x (cm)", "end y (cm)", "end z (cm)",
                                  "phi (deg)", "theta (deg)"};

const Int_t kNbins[kNdims] = {60, 60, 70, 60, 60, 70, 30, 30};
const Double_t kXmin[kNdims] = { -300, -300, -100, -300, -300, -100, -180, -180 };
const Double_t kXmax[kNdims] = { 300, 300, 600, 300, 300, 600, 180, 180 };



void track_ndhist(const char* input_file, const char* output_file) {
    ...
    for (unsigned i = 0; i < kNplanes * kNTPCs; i++) {
        h[i] = new THnSparseD(Form("htrack%d", i), "", kNdims, kNbins, kXmin, kXmax);
