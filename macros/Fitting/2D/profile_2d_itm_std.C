...
void profile_2d_itm_std(const char* input_file, const char* output_file) {
    ...
    for (int tpc = 0; tpc < kNTPCs; tpc++) {
        for (int plane = 0; plane < kNplanes; plane++) {
            ...
            std::string num_str = "h2D"+std::to_string(idx); // convert int to string
            ...
            TH2D* h_result_temp = h->Projection(1, 0);
            h_result_temp->Reset();

            TH2D* h_charge = (TH2D*)h_result_temp->Clone(Form("hQ%d", idx));
            TH2D* h_width = (TH2D*)h_result_temp->Clone(Form("hW%d", idx));
            TH2D* h_good = (TH2D*)h_result_temp->Clone(Form("hG%d", idx));

    	    // Get ITM for each slice in the specified dimension
            for (int i = 1; i < h->GetAxis(0)->GetNbins() + 1; i++) {
              for (int j = 1; j < h->GetAxis(1)->GetNbins() + 1; j++) {
                h->GetAxis(0)->SetRange(i, i);
                h->GetAxis(1)->SetRange(j, j);

                TH1D* h_1d_q = h->Projection(2); // charge
                TH1D* h_1d_w = h->Projection(3); // width 
                TH1D* h_1d_g = h->Projection(4); // goodness
	        ...
	        h_charge->SetBinContent(i, j, itm_resultQ[0]);
                h_charge->SetBinError(i, j, itm_resultQ[1]);
	        //h_charge->SetBinContent(i, silly_corr(plane)*itm_resultQ[0]);
                //h_charge->SetBinError(i, silly_corr(plane)*itm_resultQ[1]);
                h_width->SetBinContent(i, j, itm_resultW[0]);
                h_width->SetBinError(i, j, itm_resultW[1]);
                h_good->SetBinContent(i, j, itm_resultG[0]);
                h_good->SetBinError(i, j, itm_resultG[1]);
