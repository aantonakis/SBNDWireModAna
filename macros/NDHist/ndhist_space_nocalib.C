...
const UInt_t kNdims = 5;
const Float_t kTrackCut = 60.; // cm

// I think the idea of this macro is to bin very finely and to deal with stats by coarse graining later

const TString kLabels[kNdims] = { "x", "y", "z", "width", "integral"};
const TString kTitles[kNdims] = { "x (cm)", "y (cm)", "z (cm)", "Width (ticks)", "integral (Q)" };
const Int_t kNbins[kNdims] = { 600, 600, 700, 200, 2500};
const Double_t kXmin[kNdims] = { -300, -300, -100, 0, 0};
const Double_t kXmax[kNdims] = { 300, 300, 600, 20, 5000};



void ndhist_space_nocalib(const char* input_file, const char* output_file) {
	      ...
	      unsigned short east_tpc = 0;
                ...
                for (size_t i = 0; i < x[ip].GetSize(); i++) {
                    ...
                    */
                    ...
                    Double_t val[kNdims]  = {
                        sp.X(), sp.Y(), sp.Z(), width[ip][i], integral[ip][i] 
                    };
