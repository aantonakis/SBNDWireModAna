...
object_name = "hHit0"  # name of your THnSparseD object

dims = array.array('i', [0, 3, 4, 5, 7])
...
for chunk_idx in range(n_chunks):
    ...
    if chunk_idx == 0:
        final_merged = merged_chunk.Projection(len(dims), dims)
        #final_merged.SetDirectory(0)
    else:
        h_temp = merged_chunk.Projection(len(dims), dims)
        final_merged.Add(h_temp)
...
# --- Step 3: merge all partial files together ---
#print(f"Merging {len(partial_files)} partial merged files...")
#final_merged = merge_local_files(partial_files, "merged")
#print("Final merge complete.")

# --- Step 4: Project to TH3D (optional) ---
#dims = (0, 2, 3)
#print(f"Projecting to TH3D with dimensions {dims}...")
#proj = final_merged.Projection(3, dims)
#proj.SetName("merged_projection")
