...
cp -r ${filesFromSender}/single_dim_tpc_grid_TH1D.C .
...
#root -l -b -q "single_dim_tpc_grid_TH1D.C(\"input_list_${nProcess}.txt\", \"${nProcess}\", true, true, true, true, false, 2)" &> log_${nProcess}.log
root -l -b -q "single_dim_tpc_grid_TH1D.C(\"input_list_${nProcess}.txt\", \"${nProcess}\", true, true, true, true, true, 2)" &> log_${nProcess}.log
...
outFILE=${thisOutputCreationDir}/output/root/output_single_dim_tpc_TH1D_${nProcess}.root
#outFILE=${thisOutputCreationDir}/${outDir}/output_ndhist_charges_tpc_crossers_${nProcess}.root
if [ -f "$outFILE" ]; then
  echo "ifdh cp ${thisOutputCreationDir}/output/root/output_single_dim_tpc_TH1D_${nProcess}.root ${outDir}/${DFPREFIX}_${nProcess}.root"
  ifdh cp ${thisOutputCreationDir}/output/root/output_single_dim_tpc_TH1D_${nProcess}.root ${outDir}/${DFPREFIX}_${nProcess}.root
