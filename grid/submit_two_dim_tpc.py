...
def run_grid(inputfiles):
    ...
    OutputDir = SBNDCALIB_GRID_OUT_DIR + "/two_dim_tpc/" + args.output + "__" + timestamp
    ...
    os.system('cp ../grid/bin/grid_executable_two_dim_tpc.sh %s' %MasterJobDir)
    ...
    cp_script = "cp " + WIREMOD_WORKING_DIR + "/macros/LowDim/two_dim_tpc_grid.C " + MasterJobDir
...
"file://$(pwd)/grid_executable_two_dim_tpc.sh" \\
"%s" \\
